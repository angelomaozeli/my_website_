...
We see that all the DJIA stocks are following similar patterns for monthly returns. It seems reasonable as stock performance is highly correlated with market conditions. For example, during expansion periods, equity market would normally go through a “bull market”, so it would be easy for the stocks to generate returns, while in bear markets, such returns are normally lower and can be negative even. This thus results into the similar distributions we are seeing.

It is also worth noticing that even though stocks are following similar patterns, there are still differences in the exact returns they generate even at the same time. This can be quantified as “beta”, which is the correlation with the generally equity market. For example, for stocks with beta of 1, their movements are exactly with the market, while for stocks with beta of 2, the price movements would be the twice of market volatility.
...
```
However from 2019 to 2020, even though risks further increased (shifted to the right), returns seems to stay at the same level with 2019, or even lower. This is possibly due to the unexpected global COVID-19 pandemic, which caused negative market sentiments and thus large risk/volatility of the equity market.

For some investors, they may want to pull out their investments from equity market, as the additional risks they take are not compensated accordingly.
From 2020 to 2021, both returns and risks generally decrease, which suggests a calm-down equity market, by comparing to 2020. Also, 2017 seems to be the most suitable year for equity investing, due to the relatively higher level of returns while lower level of risks.